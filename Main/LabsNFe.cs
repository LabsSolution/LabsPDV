...
namespace Labs.Main
{
    public class LabsNFe
    {
		...
		// Final do Transp
		public static async void EmitirNotaFiscalDeConsumidor()
		{
			var xml = new EnviNFe
			{
				Versao = "4.00",
						...
						InfNFe = new List<InfNFe>
						{
							new InfNFe
							{
								...
								Ide =  new Ide // Objeto de Identificação da Nota
								{
									...
									NNF = 21, // Numero da Nota Fiscal (Incremental)
								...
								Det = new List<Det>
								{
									new Det
									{
										...
										Prod = new Prod
										{
											...
											NCM = "84714900", // Nomenclatura comum do Mercosul (Geralmente retorna pela consulta GTIN) 
											CFOP = "5102", // Código Identificador de Operação do Estabelecimento ();
											UCom = "UN", // Unidade Comercial
											QCom = 1.00m, // Quantidade Comercial (aparentemente é a quantidade vendida do produto)
											VUnCom = 100m, // Valor da unidade comercial (valor da unidade vendida)
											VProd = 100, // Valor do Produto (valor final do produto)
											CEANTrib = "SEM GTIN", // Código GTIN da Unidade Tributável, Caso Contrário Informar SEM GTIN
											UTrib = "UN", // Unidade Tributária
											QTrib = 1.00m, // Quantidade Tributária (pelo visto é a quantidade tributável do produto?)
											VUnTrib = 100m, // Valor da Unidade Tributável
										...
										Imposto = new Imposto
										{
											ICMS = new ICMS
											{
												//ICMSSN102 = new ICMSSN102 // Aparentemente esse é o único modelo que funciona no RJ
												//{
												...
												ICMS00 = new ICMS00
												{
													Orig = OrigemMercadoria.Nacional,
													CST = "00",
													ModBC = ModalidadeBaseCalculoICMS.MargemValorAgregado, // Usado para a aliquota Regional usando o regime normal de tributação
													VBC = 100, // Valor total do produto
													PICMS = 18.0, // Aliquota do icms
													VICMS = 100 * 0.18, //Valor do icms = vProd * aIcms.
												},
											},
											PIS = new PIS // ?
											{
								...
								Total = new Total // Total de todos os itens listados na nota (Inclui o valor de todos os itens)
								{
									ICMSTot = new ICMSTot
									{
										VBC = 100,            // Base de cálculo do ICMS (soma do valor dos produtos)
										VICMS = 100*0.18,     // Valor total do ICMS (valor total dos ICMS calculados Juntos)
										VICMSDeson = 0,       // Valor do ICMS desonerado
										VFCP = 0,             // Valor do Fundo de Combate à Pobreza (FCP)
										VBCST = 0,            // Base de cálculo do ICMS Substituição Tributária
										VST = 0,              // Valor do ICMS Substituição Tributária
										VFCPST = 0,           // Valor do FCP sobre o ICMS ST
										VFCPSTRet = 0,        // Valor do FCP retido anteriormente por Substituição Tributária
										VProd = 100,          // Valor total dos produtos/serviços
										...
										VNF = 100,            // Valor total da Nota Fiscal
									}
								},
								//
			...
			var Auto = new Unimake.Business.DFe.Servicos.NFCe.Autorizacao(xml, config); // Aqui usamos o namespace para trabalhar com a autorização da NFc-e															//
			Auto.Executar();
			//
			if (Auto.Result.ProtNFe != null)
			{
				Modais.MostrarInfo(Auto.Result.ProtNFe.InfProt.CStat.ToString());
				Modais.MostrarInfo($"URL Consulta: {Auto.Configuracoes.UrlQrCodeHomologacao}");
				switch (Auto.Result.ProtNFe.InfProt.CStat)
				{
					case 100: // NFE Autorizada
						...
						Modais.MostrarInfo(Auto.Result.ProtNFe.InfProt.ChNFe);
						Modais.MostrarInfo(Auto.Result.ProtNFe.InfProt.CMsg);
						// Geramos o xml e salvamos no banco de dados para facilitar e assegurar o nosso cliente.
						var x = Auto.Result.GerarXML();//
						var t = new NotaFiscalXml { XML = x.OuterXml };
